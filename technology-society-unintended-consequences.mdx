...
```mermaid
flowchart TD
    A[1st Order: intended\nPersonal transportation] --> B[Suburbs and commuting]
    A --> C[Highway construction]
    A --> D[Oil dependence]
    A --> E[Air pollution]
    B --> F[Urban sprawl]
    D --> G[Geopolitical oil conflicts]
    E --> H[Climate change]
    B --> I[Sedentary lifestyles]
    F --> J[Social isolation]
...
Each order is harder to predict and harder to reverse.
