...
```text
  ...
  Reality        Simplify        Model
  (complex)  --> (assumptions) --> (equations)
     ^                              |
     |                            Solve
     |                              |
     |                              v
  Validate     <-- Compare <--  Prediction
  (measure real     (does it     (model
   hardware)        match?)       output)

  If prediction matches reality: useful model
  If prediction diverges: revise assumptions
```
